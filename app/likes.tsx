...
export default function Likes({ tweet, addOptimisticTweet }: { tweet: TweetWithAuthor; addOptimisticTweet: (newTweet: TweetWithAuthor) => void }){
  ...
  return(
    <button onClick={handleLikes} className="group flex items-center">
      <svg className={`group-hover:fill-red-400 group-hover:stroke-red-400 ${tweet.user_has_liked_tweet ? "fill-red-600 stroke-red-600" : "fill-none stroke-gray-500"}`} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
      <span className={`group-hover:text-red-500 ml-2 text-sm ${tweet.user_has_liked_tweet ? "text-red-500" : "text-gray-500"}`}>{tweet.likes}</span>
    </button>
  )
}